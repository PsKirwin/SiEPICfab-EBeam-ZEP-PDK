...
# 2020/12: process currently only uses high resolutio
# added boolean operations for rib waveguides pkirwin 2026/01
HighandLowRes = True
...
def boolean_layer_operations(topcell_in, flatten=False):
    ...
    layer_etch_partial = layout.layer(pya.LayerInfo(102, 0))  # added for rib waveguides pkirwin 2026/01
    layer_core=layout.layer(pya.LayerInfo(1, 0))
    layer_clad = layout.layer(pya.LayerInfo(1, 2))
    layer_slab = layout.layer(pya.LayerInfo(2, 0))  # added for rib waveguides pkirwin 2026/01
    layer_temp = layout.layer(pya.LayerInfo(1, 455))
    layer_temp2 = layout.layer(pya.LayerInfo(1, 456))
    layer_temp3 = layout.layer(pya.LayerInfo(1, 457))
    ...
    for i in range(0,layout.cells()):
        ...
        shapes_etch_partial = cell.shapes(layer_etch_partial)  # added for rib waveguides pkirwin 2026/01
        shapes_core = cell.shapes(layer_core)
        shapes_clad = cell.shapes(layer_clad)
        shapes_slab = cell.shapes(layer_slab)  # added for rib waveguides pkirwin 2026/01
        shapes_temp = cell.shapes(layer_temp)
        shapes_temp2 = cell.shapes(layer_temp2)
        shapes_temp3 = cell.shapes(layer_temp3)
        ...
        # For rib waveguides: NOT core with slab to get the rib only
        pya.ShapeProcessor().boolean(
            layout,cell,layer_core,
            layout,cell,layer_slab,
            shapes_etch_partial,pya.EdgeProcessor.ModeBNotA,False,True,True)
        
        # then not the slab with clad to make sure they don't overlap
        pya.ShapeProcessor().boolean(
            layout,cell,layer_etch_partial,
            layout,cell,layer_temp2,
            shapes_temp3,pya.EdgeProcessor.ModeBNotA,False,True,True) 
...
#        ['GC_1290_te_220_Blackbox','GC_1290_te_220', 'SiEPICfab_EBeam_ZEP_UBC'], 
    ...
    save_options.add_layer(ly.layer(LayerInfo(102,0)), LayerInfo())# Si etch partial (rib waveguides)  pkirwin 2026/01
